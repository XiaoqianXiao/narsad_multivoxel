...
#   ./submit_mvpa_L2_fearnetwork_stage.sh post14
#   ./submit_mvpa_L2_fearnetwork_stage.sh 11:SAD
#   ./submit_mvpa_L2_fearnetwork_stage.sh 12
#   ./submit_mvpa_L2_fearnetwork_stage.sh 12 --n_permutation 1000
#   ./submit_mvpa_L2_fearnetwork_stage.sh --help
...
case "$STAGE11_MASK_MODE" in
  current)
    DEFAULT_OUT_DIR="/output_dir/FearNetwork"
    ;;
  original_notebook)
    DEFAULT_OUT_DIR="/output_dir/FearNetwork_originalMask"
    ;;
  *)
...
LOG_DIR="${LOG_DIR:-${PROJECT_ROOT}/logs/mvpa_l2_fearnetwork}"
...
Usage:
  ...
  submit_mvpa_L2_fearnetwork_stage.sh post<N> [extra python args]
  ...
  submit_mvpa_L2_fearnetwork_stage.sh 11:SAD
  submit_mvpa_L2_fearnetwork_stage.sh 11:HC
...
Examples:
  submit_mvpa_L2_fearnetwork_stage.sh 6
  submit_mvpa_L2_fearnetwork_stage.sh post14
  submit_mvpa_L2_fearnetwork_stage.sh post24
  submit_mvpa_L2_fearnetwork_stage.sh 11:SAD
  submit_mvpa_L2_fearnetwork_stage.sh 11:HC
  submit_mvpa_L2_fearnetwork_stage.sh 12 --n_permutation 1000
  N_PERMUTATION=1000 CPUS=8 N_JOBS=8 submit_mvpa_L2_fearnetwork_stage.sh all
  STAGE11_CHUNKS=200 submit_mvpa_L2_fearnetwork_stage.sh 11:SAD
  STAGE11_CHUNKS=100 STAGE11_CHUNK_IDX=80 submit_mvpa_L2_fearnetwork_stage.sh 11:SAD
...
Stage / cell reference:
  ...
  10  Voxel-wise spatial/Haufe analysis in the FearNetwork ROI set.
...
Environment overrides:
  PROJECT_ROOT, CONTAINER_SIF, APP_PATH, OUT_BASE, OUT_DIR, FEAR_ROI_DIR
...
submit_post_stages_after() {
  ...
  for stage in "${POST_STAGE11_STAGES[@]}"; do
    if (( stage > after_stage )); then
      ...
      if [[ -n "$prev_job" ]]; then
        echo "Submitted FearNetwork stage ${stage} after job ${prev_job}: job ${job_id}"
      else
  ...
  echo "Submitted chained FearNetwork stages after stage ${after_stage}."
}
...
if [[ "$REQUESTED" == "ALL" ]]; then
  ...
  echo "Submitted chained FearNetwork stages with split stage 11 SAD/HC jobs."
