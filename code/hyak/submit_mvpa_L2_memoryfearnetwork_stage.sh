...
Usage:
  ...
  submit_mvpa_L2_memoryfearnetwork_stage.sh <stage_id> [extra python args]
...
submit_stage() {
  ...
  job_id=$(
    sbatch --parsable \
      ...
      --mem="$MEM" \
      --time="$TIME" \
      --job-name="mvpa_memfear_${job_suffix}" \
      --output="$LOG_DIR/mvpa_memfear_${job_suffix}_%j.out" \
      --error="$LOG_DIR/mvpa_memfear_${job_suffix}_%j.err" \
...
submit_stage11_array() {
  ...
  job_id=$(
    sbatch --parsable \
      ...
      --mem="$MEM" \
      --time="$TIME" \
      --job-name="mvpa_memfear_$(stage_output_suffix 11)_${group_name}" \
      --output="$LOG_DIR/mvpa_memfear_$(stage_output_suffix 11)_${group_name}_%A_%a.out" \
      --error="$LOG_DIR/mvpa_memfear_$(stage_output_suffix 11)_${group_name}_%A_%a.err" \
...
submit_stage11_chunk() {
  ...
  job_id=$(
    sbatch --parsable \
      ...
      --mem="$MEM" \
      --time="$TIME" \
      --job-name="mvpa_memfear_$(stage_output_suffix 11)_${group_name}_chunk${chunk_idx}" \
      --output="$LOG_DIR/mvpa_memfear_$(stage_output_suffix 11)_${group_name}_chunk${chunk_idx}_%j.out" \
      --error="$LOG_DIR/mvpa_memfear_$(stage_output_suffix 11)_${group_name}_chunk${chunk_idx}_%j.err" \
...
submit_stage11_merge() {
  ...
  job_id=$(
    sbatch --parsable \
      ...
      --mem="$MEM" \
      --time="$TIME" \
      --job-name="mvpa_memfear_$(stage_output_suffix 11)_${group_name}_merge" \
      --output="$LOG_DIR/mvpa_memfear_$(stage_output_suffix 11)_${group_name}_merge_%j.out" \
      --error="$LOG_DIR/mvpa_memfear_$(stage_output_suffix 11)_${group_name}_merge_%j.err" \
...
if [[ "$REQUESTED" == "ALL" ]]; then
  ...
  for group_name in "${STAGE11_GROUPS[@]}"; do
    ...
    echo "Submitted MemoryFearNetwork stage 11:${group_name} array: job ${job_id}"
    stage11_jobs+=("$job_id")
    merge_job_id="$(submit_stage11_merge "$group_name" "$job_id")"
    echo "Submitted MemoryFearNetwork stage 11:${group_name} merge: job ${merge_job_id}"
  ...
  if [[ "$REQUESTED" =~ ^11[:_-](SAD|HC)$ ]]; then
    ...
    if [[ -n "${STAGE11_CHUNK_IDX:-}" ]]; then
      ...
      echo "Submitted MemoryFearNetwork stage 11:${group_name} chunk ${STAGE11_CHUNK_IDX}/${STAGE11_CHUNKS}: job ${chunk_job_id}"
      ...
      echo "Submitted MemoryFearNetwork stage 11:${group_name} array: job ${array_job_id}"
    ...
    echo "Submitted MemoryFearNetwork stage ${REQUESTED}: job ${job_id}"
  fi
fi
