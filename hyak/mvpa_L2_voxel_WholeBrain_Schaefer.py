...
def calculate_distribution_stats(X, y, subjects, feature_mask, best_params_dict):
    ...
    for sub in unique_subs:
        ...
        try:
            ...
            probs_all = cross_val_predict(calib_model, X_binary, y_binary, groups=np.full(len(y_binary), sub), cv=cv, method='predict_proba', n_jobs=N_JOBS_CV)
...
def calc_metrics_for_subject(X, y, sub_id, feature_mask, C_param=1.0):
    ...
    try:
        ...
        probs_all = cross_val_predict(calib_model, X_bin, y_bin, groups=np.full(len(y_bin), sub_id), cv=cv, method='predict_proba', n_jobs=N_JOBS_CV)
...
# Resume support: load prior checkpoints if running a single stage
if RESUME and STAGE is not None:
    # Prefer intermediates when available; fall back to checkpoints.
    _inter_map = {
        6: ["stage06_models", "stage06_permutation", "stage06_maps"],
        7: ["stage07_spatial_results"],
        8: ["stage08_importance"],
        9: ["stage09_results_12"],
        10: ["stage10_results_13"],
        11: ["stage11_results_13b"],
        12: ["stage12_results_14_self"],
        13: ["stage13_results_21"],
        14: ["stage14_results_22"],
        15: ["stage15_results_23"],
        16: ["stage16_results_24"],
        17: ["stage17_rsm"],
    }
    for _cid in range(6, STAGE):
        loaded = False
        for _name in _inter_map.get(_cid, []):
            try:
...
if STAGE is None or STAGE == 6:
    ...
    save_intermediate("stage06_maps", {"map_sad": map_sad, "map_hc": map_hc, "obs_sim": obs_sim, "p_sim_spatial": p_sim_spatial})
...
if STAGE is None or STAGE == 7:
    ...
    save_intermediate("stage07_spatial_results", {"spatial_results": locals().get("spatial_results")})
