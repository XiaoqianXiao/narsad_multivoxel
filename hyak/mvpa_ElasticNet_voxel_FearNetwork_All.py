#!/usr/bin/env python3
"""
MVPA ElasticNet Voxel-wise Analysis - Fear Network All (Expert Version)
Converted from Jupyter notebook mvpa_L2_voxel_FearNetworkAll_E.ipynb to Python script.

This is the "Expert" version using ElasticNet regularization and RobustScaler for
improved handling of fMRI data with outliers and motion artifacts.

This script performs comprehensive MVPA analyses on fear network data following the exact
logic and execution order of the original notebook. All analyses are included:

ANALYSIS 1: BASELINE NEURAL REPRESENTATIONS
- Analysis 1.1 (Cell 6): Neural Dissociation
    * Self-decoding (baseline discriminability)
    * Cross-decoding (functional specificity)
    * Spatial specificity (Haufe pattern similarity)
    
- Analysis 1.2 (Cell 7/8): Spatial Topology & Visualization
    * Voxel-wise permutation testing with FDR correction
    * ROI reconstruction and glass brain visualization
    
- Analysis 1.2 (Cell 8): Feature Importance
    * Permutation importance masks for SAD and HC groups
    * Identification of task-relevant voxels
    
- Analysis 1.2 (Cell 9): Static Representational Topology
    * Top 5% feature selection
    * Centroid-based Representational Dissimilarity Matrices (RDMs)
    * Statistical tests for threat and safety distances
    
- Analysis 1.3 (Cell 10): Dynamic Representational Drift (Top 5% Features)
    * Safety learning: CSS(Ext) -> CS-(Ext)
    * Threat maintenance: CSR(Ext) -> CSR(Reinst)
    * Projection and cosine similarity metrics
    
- Analysis 1.3 (Cell 11): Single-Trial Trajectories
    * Trial-by-trial plasticity visualization
    * Statistical testing across trials
    
- Analysis 1.4 (Cell 12): Decision Boundary Characteristics
    * Cognitive Certainty (Entropy)
    * Decision Sharpness (Kurtosis)
    * Probability distribution variance

ANALYSIS 2: OXYTOCIN EFFECTS
- Analysis 2.1 (Cell 13): Safety Restoration & Threat Discrimination
    * Linear Mixed Effects (LME) models
    * Safety restoration: CSS vs CS- distance
    * Threat discrimination: CSR vs CSS distance
    
- Analysis 2.2 (Cell 14): Drift Efficiency
    * LME models for drift metrics
    * Safety and threat maintenance efficiency
    
- Analysis 2.3 (Cell 15): Probabilistic Opening
    * LME models for Entropy, Kurtosis, Variance
    * Tests for increased cognitive uncertainty in SAD
    
- Analysis 2.4 (Cell 16): Spatial Re-Alignment
    * Cross-decoding: HC-trained model -> SAD groups
    * Tests for "normalizing" effect of Oxytocin
    
- Analysis 2.5 (Cell 17): Reverse Cross-Decoding
    * SAD-trained model -> HC groups
    * Tests for functional specificity of disordered representations

All outputs and figures are saved to the specified output_dir.

Usage:
    python mvpa_L2_voxel_FearNetworkAll_E.py --output_dir /path/to/output [--project_root /path/to/project] [--skip_analyses 1.1 1.2 1.3 1.4 2.1 2.2 2.3 2.4 2.5]

Key Differences from L2 Version:
    - Uses ElasticNet regularization (combines L1 sparsity + L2 grouping)
    - Uses RobustScaler instead of StandardScaler (better for fMRI outliers)
    - Tunes both C and l1_ratio hyperparameters
    - Optimized for cluster computing (N_JOBS=32)
    - Uses 'saga' solver with increased max_iter (10000)

Arguments:
    --output_dir: Optional. Directory where all results and figures will be saved.
    --project_root: Optional. Root directory of the project (default: /Users/xiaoqianxiao/projects/NARSAD)
    --skip_analyses: Optional. Space-separated list of analyses to skip (e.g., "1.1 2.3")

Output Files:
    - analysis_1_1_neural_dissociation.png: Self/cross-decoding and spatial specificity results
    - analysis_1_2_spatial_*.png: Voxel-wise spatial maps for SAD and HC
    - analysis_1_2_static_topology.png: RDM heatmaps and distance metrics
    - analysis_1_3_dynamic_drift.png: Drift efficiency bar plots
    - analysis_1_3_trajectories.png: Single-trial trajectory plots
    - analysis_1_4_decision_boundary.png: Entropy, kurtosis, and probability distributions
    - analysis_2_1_safety_restoration.png: Safety and threat distance comparisons
    - analysis_2_2_drift_efficiency.png: Drift efficiency by drug condition
    - analysis_2_3_probabilistic_opening.png: Decision boundary metrics by drug
    - analysis_2_4_spatial_realignment.png: Cross-decoding accuracy heatmap
    - analysis_2_5_reverse_crossdecoding.png: Reverse cross-decoding accuracy
    - *.pkl files: Pickled results for each analysis
    - importance_scores.pkl: Feature importance scores
    - models.pkl: Trained models from Analysis 1.1
    - top5_masks.pkl: Top 5% feature masks

Data Requirements:
    - phase2_X_ext_y_ext_roi_voxels.npz: Extinction phase data
    - phase3_X_reinst_y_reinst_roi_voxels.npz: Reinstatement phase data
    - drug_order.csv: Subject metadata (Group, Drug)
    - ROI masks: Located at ROI_DIR (default: /Users/xiaoqianxiao/tool/parcellation/Gillian_anatomically_constrained)

Note: This script follows the exact same logic and execution order as the original notebook.
      All analyses are implemented with the same methods, parameters, and statistical tests.
"""
...
N_JOBS = 32  # Optimized for cluster computing
thresh_hold_p = 1 - 0.05
N_PERMUTATION = 5000
N_REPEATS = 10

# ElasticNet Configuration (Expert Version)
...
ROI_DIR = '/gscratch/fang/NARSAD/ROI/Gillian_anatomically_constrained'
ROI_ORDER = TARGET_ROIS
...
def build_binary_pipeline():
    ...
    return Pipeline([
        ...
        ('classification', LogisticRegression(
            ...
            solver=SOLVER_TYPE,
            l1_ratio=0.5,        # Placeholder; MUST be tuned in param_grid
            class_weight='balanced',
            max_iter=MAX_ITER_SAGA,
            random_state=RANDOM_STATE,
            n_jobs=1  # Keep at 1 if inner GridSearchCV is already using n_jobs
        ))
    ])
...
def run_pairwise_decoding_analysis(X, y, subjects, n_repeats=10):
    ...
    for c1, c2 in pairs:
        ...
        # We use biased=False for covariance to stay conservative (ddof=1)
        ...
        results[pair_name] = {
            ...
            'best_C': gs_final.best_params_['classification__C'],  # For backward compatibility
...
def reconstruct_roi_map(flat_data, roi_names, roi_dir):
    """Paints a 1D array of values back into a 3D brain volume."""
    # 1. Determine Reference Space
    ...
    for name in roi_names:
        ...
        final_vol[mask_data] = roi_values
        
        current_idx += n_voxels
        
    if current_idx != len(flat_data):
         print(f"  ! Warning: {len(flat_data) - current_idx} features were unused.")
...
def run_wen_paper_analysis_voxelwise(X, y, subjects, pipeline_template, best_params, n_permutations, fdr_alpha=0.05):
    """
    Updated for ElasticNet and High-Performance Computing.
    best_params: should be the dictionary from gs.best_params_ (contains both C and l1_ratio)
    """
    print(f"  Estimating Weights ({n_permutations} perms) with ElasticNet...")
    pipe = clone(pipeline_template)
    # Set BOTH C and l1_ratio from best_params
    ...
    # Use 'loky' backend for Scikit-Learn on clusters (safer than default)
...
def calculate_distribution_stats(X, y, subjects, feature_mask, best_params_dict, COND_CLASS_THREAT="CSR", COND_CLASS_SAFE="CSS"):
    ...
    for sub in unique_subs:
        ...
        try:
            ...
            # For ElasticNet, handle both dict and scalar C values
            if isinstance(c_val, dict):
                fixed_model.set_params(**c_val)
            else:
...
def calc_metrics_for_subject(X, y, sub_id, feature_mask, C_param=1.0, COND_CLASS_THREAT="CSR", COND_CLASS_SAFE="CSS"):
    ...
    try:
        ...
        # For ElasticNet, we need to set both C and l1_ratio
        # If C_param is a dict, use it; otherwise assume default l1_ratio
        if isinstance(C_param, dict):
            model.set_params(**C_param)
        else:
...
def analysis_1_1_neural_dissociation(data_subsets, output_dir):
    ...
    perm_pipe.set_params(classification__C=1.0, classification__l1_ratio=0.5)
    ...
    for sub in np.unique(sub_comb):
        ...
        cov = np.cov(perm_pipe.named_steps['scaler'].transform(X_comb[mask]), rowvar=False, ddof=1)
        A = cov @ W.T
...
def analysis_1_2_spatial_topology(data_subsets, res_sad_dict, res_hc_dict, output_dir):
    ...
    for name, data in groups.items():
        ...
        z_scores, sig_mask = run_wen_paper_analysis_voxelwise(
            X_p, y_curr, sub_curr, build_binary_pipeline(), data['res']['best_params'], N_PERMUTATION, fdr_alpha
        )
...
def analysis_1_3_dynamic_drift(data_subsets, X_ext, y_ext, sub_ext, X_reinst, y_reinst, sub_reinst, importance_scores, output_dir, X_ext_global=None, y_ext_global=None, sub_ext_global=None):
    """Analysis 1.3: Dynamic Representational Drift (Top 5% Features) - Cell 10"""
    ...
    # Use global X_ext for CS- target (safety analysis needs CS- which was filtered out in Cell 5)
    # Pass X_ext, y_ext, sub_ext as parameters
...
def analysis_1_3_trajectories(data_subsets, X_ext, y_ext, sub_ext, X_reinst, y_reinst, sub_reinst, importance_scores, output_dir, X_ext_global=None, y_ext_global=None, sub_ext_global=None):
    """Analysis 1.3: Single-Trial Trajectories (Top 5%) - Cell 11"""
    ...
    # Use global X_ext for CS- target
...
def analysis_2_5_reverse_crossdecoding(data_subsets, importance_scores, output_dir):
    ...
    acc_hc_plc = calc_forced_choice_acc(sad_model, X_hc_plc, y_hc_plc, sub_hc_plc, mask_sad_native, COND_THREAT, COND_SAFE, LABELS)
    acc_hc_oxt = calc_forced_choice_acc(sad_model, X_hc_oxt, y_hc_oxt, sub_hc_oxt, mask_sad_native, COND_THREAT, COND_SAFE, LABELS)
...
def main():
    parser = argparse.ArgumentParser(description='MVPA L2 Voxel-wise Analysis - Fear Network All')
    parser.add_argument('--project_root', type=str, 
                       default='/Users/xiaoqianxiao/projects/NARSAD',
                       help='Root directory of the project')
    parser.add_argument('--output_dir', type=str,
                      default='/Users/xiaoqianxiao/projects/NARSAD/MRI/derivatives/fMRI_analysis/LSS/outputs/ElasticNet',
    ...
    # Load data
    X_ext, y_ext, sub_ext, X_reinst, y_reinst, sub_reinst, parcel_names_ext = load_data(args.project_root, args.output_dir)
    ...
    # Run analyses in exact order as notebook
    skip_list = args.skip_analyses if args.skip_analyses else []
    
    # Initialize variables (matching notebook structure)
    res_sad_dict = res_hc_dict = None
    importance_scores = None
    importance_masks = None
    mask_sad_top5 = mask_hc_top5 = None
    subject_best_params = {}  # Default empty dict as in notebook
    
    # Analysis 1.1: Neural Dissociation (Cell 6)
    if '1.1' not in skip_list:
        print("\n" + "="*80)
        print("CELL 6: ANALYSIS 1.1 - Neural Dissociation")
        print("="*80)
    ...
    if '1.2' not in skip_list and res_sad_dict is not None:
        ...
        print("\n" + "="*80)
        print("CELL 7/8: ANALYSIS 1.2 - Spatial Topology (Voxel-wise)")
        print("="*80)
